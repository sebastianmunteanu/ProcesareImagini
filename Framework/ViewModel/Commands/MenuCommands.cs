...
namespace Framework.ViewModel
{
    public class MenuCommands : BaseVM
    {
        ...
        public ICommand MirrorImageVertically
        {
            get
            {
                if (_mirrorImageVertically == null)
                    _mirrorImageVertically = new RelayCommand(MirrorImageVerticallyMethod);
                return _mirrorImageVertically;
            }
        }

        private void MirrorImageVerticallyMethod (object parameter)
        {
            if (InitialImage == null)
            {
        ...
        #region BinaryImage
        private ICommand _binaryImage;

        public ICommand BinaryImage
        {
            get
            {
                if (_binaryImage == null)
                    _binaryImage = new RelayCommand(BinaryImageMethod);
                return _binaryImage;
            }
        }

        public void BinaryImageMethod (object parameter)    
        {
            if (InitialImage == null)
            {
                MessageBox.Show("Intoduceti o imagine");
            ...
            paramms.Add("Threshold: ");

            DialogBox db = new DialogBox(_mainVM, paramms);
            db.ShowDialog();
            ...
            if (results != null)
            {
                ...
                if (GrayInitialImage != null)
                {
                    GrayProcessedImage = Tools.BinaryImage(GrayInitialImage, thresHold);
                    ProcessedImage = Convert(GrayProcessedImage);
                }
                ...
                {
                    ColorProcessedImage = Tools.BinaryImage(ColorInitialImage, thresHold);
                    ProcessedImage = Convert(ColorProcessedImage);
                }
            }   
        ...
        public ICommand CropImage
        {
            ...
            {
                if (_cropImage == null)
                    _cropImage = new RelayCommand(CropImageMethod);
                return _cropImage;
            }
        }

        private void CropImageMethod(object parameter)
        {
            if (InitialImage == null)
            {
                MessageBox.Show("Intoduceti o imagine");
