...
namespace Algorithms.Tools
{
    public class Tools
    {
        ...
        #endregion Miror Image Vertically
        ...
        public static Image<Gray, byte> BinaryImage(Image<Gray, byte> inputImage, int threshold)
        {
            Image<Gray, byte> result = new Image<Gray, byte>(inputImage.Size);
            for (int y = 0; y < result.Height; y++)
            {
        ...
        public static Image<Bgr, byte> BinaryImage(Image<Bgr, byte> inputImage, int threshold)
        {
            Image<Bgr, byte> result = new Image<Bgr, byte>(inputImage.Size);
            for (int y = 0; y < result.Height; y++)
            {
                for (int x = 0; x < result.Width; x++)
                {
                    int intermediar = (inputImage.Data[y, x, 0] + inputImage.Data[y, x, 1] + inputImage.Data[y, x, 2]) / 3;
                    if (intermediar >= threshold)
                    {
                        result.Data[y, x, 0] = 0;
                        result.Data[y, x, 1] = 0;
                        result.Data[y, x, 2] = 0;
        ...
        #region CropImage
        public static Image<Gray, byte> CropImage(Image<Gray, byte> inputImage, Tuple<int, int> top, Tuple<int, int> bottom)
        {
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                {
                    result.Data[y, x, 0] = inputImage.Data[y + top.Item2, x + top.Item1, 0];
                }
            }
            return result;
        }
